...
Draggable.create("#add_new", {type:"x,y", edgeResistance:0.65, bounds:".zoomwrapper", throwProps:true});

//Draggable.create("#add_new", {type: "rotation", throwProps: true});
...
TweenLite.set("#add_new", {width:gridWidth, height:gridHeight, lineHeight:gridHeight + "px"});

//the update() function is what creates the Draggable according to the options selected (snapping).
function update() {
  var snap = $snap.prop("checked"),
      liveSnap = $liveSnap.prop("checked");
	Draggable.create("#add_new", {
		bounds:$zoomwrapper,
		edgeResistance:0.65,
		type:"x,y",
		throwProps:true,
...
function applySnap() {
	if ($snap.prop("checked") || $liveSnap.prop("checked")) {
		$("#add_new").each(function(index, element) {
